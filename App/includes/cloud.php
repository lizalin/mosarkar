...
class PTagCloud {
    ...
    function keywords_extract($text) {
        ...
        //Added By Abhiram Samantara On:03-07-2017 For Excluding Words from word cloud

        $excludedWords = file_get_contents(URL . 'includes/excludedWords.php');
        $excludedWordsArr = json_decode($excludedWords, true);
        $j = 0;
        $exArr = $excludedWordsArr['word'];

        foreach ($exArr as $exArrs) {
            $commonWords .= ',' . $exArrs;
            $j++;
        }
    ...
    function emitCloud($bHTML = true) {
        ...
        if (is_array($this->m_arTags)) {
            ...
            foreach ($arTopTags as $tag => $useCount) {
                ...
                $dataL[$j]['text'] = $tag;
                $dataL[$j]['weight'] = $grade;
                $dataL[$j]['link'] = $this->m_searchURL . urlencode($tag);
                $j++;
            }
            return $dataL;
        }
    ...
    function tagShowO($limit) {
        arsort($this->m_arTags);
        $arTopTags = array_slice($this->m_arTags, 0, $this->m_displayedElementsCount);

        /* randomize the order of elements */
        uasort($arTopTags, 'ds_sort');
