...
<script>
  ...
  ?>
  <?php
  if($_SESSION['adminConsole_Desg_Id']==CMO_DESIGNATION){
  ?>
  var downloadExcel = 0;
  <?php
  }else{
  ...
  $(document).ready(function(){
      $("#demo-form-checkbox").prop('checked', true);
      $("#demo-form-checkbox-2").prop('checked', true);
      $("#demo-form-checkbox-4").prop('checked', true);
      $("#demo-form-checkbox-5").prop('checked', true);
      $("#demo-form-checkbox-3").prop('checked', true);
      $("#callType").val(1);
      var deptSel =[];
      <?php if($deptLogin == 0 || $deptLogin== RCMS){ ?>
           deptSel.push(<?php echo(RCMS); ?>);
       <?php } ?>
       <?php if($deptLogin == 0 || $deptLogin== PADDY){ ?>
           deptSel.push(<?php echo(PADDY); ?>);
       <?php } ?>
    // console.log(deptSel);
     $("#deptSel").val(deptSel);
     
     // For Map Outbound Dynamic
     $('body').on('click', '.clsOutbound', function(){
        var outBoundId = [];
        $.each($(".clsOutbound:checked"), function(){            
            outBoundId.push($(this).val());
        });
        $("#outBoundId").val('');
        $("#outBoundId").val(outBoundId.join(","));
        getMapDataDashboard('<?php echo date('Y-m-1');?>','<?php echo date('Y-m-d');?>',1);
       // console.log(outBoundId);
     })
    $(".clsInbound").click(function(){
            var deptSel = []; 
            var callCenter ;
            //$("#callType").val(2);
            $.each($(".clsInbound:checked"), function(){     
                if($(this).val()=='3' && $(".clsInbound:checked").length>=1){
                 $("#deptSel1").val(2);
                }else if($(this).val()!='3' && $(".clsInbound:checked").length>=1){   
                     $("#deptSel1").val(0);
                    deptSel.push($(this).val());
                }
                console.log($("#deptSel1").val());
            });
            console.log($(".clsInbound:checked").length);
            if($(".clsInbound:checked").length==0){
              $("#deptSel1").val(0);
            }else if($(".clsInbound:checked").length==3){
              $("#deptSel1").val(2);
            }
            //alert(deptSel.join(", "));
            $("#deptSel").val('');
            $("#deptSel").val(deptSel.join(","));
            //callType = $("#callType").val();
             getMapDataDashboard('<?php echo date('Y-m-1');?>','<?php echo date('Y-m-d');?>',2);
            //alert("My favourite sports are: " + deptSel.join(", "));
        });

    // $(".magic-checkbox1").click(function(){
    //         var deptSel1;
    //         var callType = $("#callType").val();
    //         $.each($("input[name='mapFilterCheck1']:checked"), function(){            
    //             deptSel1=$(this).val();
    //         });
    //         //alert(deptSel1);
    //          $("#deptSel").val('12,14');
    //          $("#deptSel1").val('');
    //          $("#deptSel1").val(deptSel1);
    //          getMapDataDashboard('<?php echo date('Y-m-1');?>','<?php echo date('Y-m-d');?>',callType);
    //         //alert("My favourite sports are: " + deptSel.join(", "));
    //     });
    ...
    getMapDataDashboard('<?php echo date('Y-m-1');?>','<?php echo date('Y-m-d');?>',1);
    getOutboundSummary('<?php echo $firstWeek;?>','<?php echo date('Y-m-d');?>');
    getInboundSummary('<?php echo $firstWeek;?>','<?php echo date('Y-m-d');?>');
//Applicable for Home dept..this function will be change if the same feature  will be applicable for other deppartment..Chinmayee
    <?php if($_SESSION['adminConsole_agency']>0){?>
     fillRangeWisePolicestation(<?php echo $_SESSION['adminConsole_agency'];?>,'intHospitalPolice',0);
     fillDistrictRangeWise(<?php echo $_SESSION['adminConsole_agency'];?>,'intDistrictId',0);
    //  $(document).find('#intDistrictId').on('Change',function(){
    //   var intDistrictId =$(this).val();
    //   alert(intDistrictId);
    //   if(intDistrictId>0){
    //     fillRangeWisePolicestation(<?php echo $_SESSION['adminConsole_agency'];?>,'intHospitalPolice',0);
    //   }else {
    //      getHospitalPolish($('#intDepartmentId').val(),intDistrictId);
    //   }
    //  });
...
<?php } ?>
  ...
  //   getOutboundSummary((vchFromDate)?vchFromDate:'2019-09-20',(vchToDate)?vchToDate:'2019-09-30');
  //   getInboundSummary((vchFromDate)?vchFromDate:'2019-09-20',(vchToDate)?vchToDate:'2019-09-30');
  // }
  ...
  function getHospitalPolish(departmentId, districtId){
    if(departmentId == 12){
      getDeptWiseHospital('intHospitalPolice',districtId,0);
    }else if(departmentId == 14){
      getDeptWisePolice('intHospitalPolice',districtId,0)
    }
...
<?php 
  $home['type']='1';            
  $home['intDepartmentId']=RCMS;            
  $homeEncrypt   = $objClsBind->encrypt(json_encode($home));
  $health['type']='1';            
  $health['intDepartmentId']=PADDY;            
  $healthEncrypt   = $objClsBind->encrypt(json_encode($health));
  $inbound['type']='2';            
  $inboundEncrypt   = $objClsBind->encrypt(json_encode($inbound));

  $authCall['type']='3';            
  $authCall['intFeedbackCollectedBy']=3;            
  $authEncrypt   = $objClsBind->encrypt(json_encode($authCall));

  $ccCall['type']='3';            
  $ccCall['intFeedbackCollectedBy']=2;            
  $ccEncrypt   = $objClsBind->encrypt(json_encode($ccCall));

  $outBoundToday['type']='1';            
  $outBoundTodayEncrypt   = $objClsBind->encrypt(json_encode($outBoundToday));

  $outBoundWeek['type']='2';            
  $outBoundWeekEncrypt   = $objClsBind->encrypt(json_encode($outBoundWeek));

  $outBoundMonth['type']='3';            
  $outBoundMonthEncrypt   = $objClsBind->encrypt(json_encode($outBoundMonth));
  // echo $homeEncrypt; exit;
  ?>


<script src="https://code.highcharts.com/highcharts.js"></script>
<script src="https://code.highcharts.com/highcharts-more.js"></script>
 <link rel="stylesheet" type="text/css" href="<?php echo URL ?>css/chosen.min.css">
...
<div class="citizen-connect-section ">
   <h5 class="s-heading text-white">Citizen  Connect  </h5>
<div class="form-section">
  <div class="row form-group">
                                <div class="col-sm-6 col-md-6">
                                 <div class="input-group">
                                  <input type="text" style="position: fixed; left: -10000000px;" disabled/>
<input type="text" class="form-control datepicker"  placeholder="From Date" name="vchCitizenFromDate" id="vchCitizenFromDate" value="<?php echo date('d-M-Y', strtotime("-7 day"));?>">
<span class="input-group-addon"><i class="icon-calendar1"></i></span>
</div>
                                </div>
                                 <div class="col-sm-6 col-md-6">
                                  <div class="input-group">
<input type="text" class="form-control datepicker" placeholder="To Date" name="vchCitizenToDate" id="vchCitizenToDate" value="<?php echo date('d-M-Y');?>">
<span class="input-group-addon"><i class="icon-calendar1"></i></span>
</div>
                                </div>
...
<div class="row form-group">
                                 ...
                                 <div class="col-md-6 col-sm-6">
                                  <select class="form-control" id="intServicesId" name="intServicesId" onchange="getServiceWiseScheme('intSchemeTypeId',this.value,0);">
                               ...
                               <div class="row form-group">
                                 ...
                                 <div class="col-md-6 col-sm-6">
                                  ...
                                  <div class="row form-group">
                                  <div class="col-md-6 col-sm-6">
                                     <?php //if($_SESSION['adminConsole_agency']>0){ onchange="fillRangeWisePolicestation(<?php echo $_SESSION['adminConsole_agency'];intHospitalPolice',this.value)"  <?php }else{ onchange="getHospitalPolish($('#intDepartmentId').val(),this.value)" <?php } ?>
                                    <select class="form-control" name="intDistrictId" id="intDistrictId">
                                    <option value="0">--- Select District ---</option>
                                  </select>
                                </div>
                              <!--    <div class="col-md-6 col-sm-6">
                                  <select class="form-control" id="intHospitalPolice" name="intHospitalPolice">
                              ...
                              <label class="col-lg-3 col-md-4 col-sm-4 control-label" for="demo-email-input">Number of Calls </label>
                              <div class="col-lg-8 col-md-7 col-sm-8">
                                 <div class="radio">
                                    <input id="intGenderId_5" class="magic-radio" value="2" type="radio" name="intCallerId" >
                                    <label for="intGenderId_5">2</label> &nbsp;&nbsp;
                                    <input id="intGenderId_6" class="magic-radio" value="3" type="radio" name="intCallerId" >
                                    <label for="intGenderId_6">3</label> &nbsp;&nbsp;
                            ...
                            <div class="row form-group mt-2">
                                 ...
                                 <div class="col-md-12">
                                  <a class="btn call-btn" href="javascript:void(0);" type="button" id="searchbtn">Choose Citizen</a>
                                </div>
                               </div>
...
<div class="portlet height442-list caller-list">
  <div class="portlet__header">
    Citizen List
    <a class="btn btn-warning pull-right" id="make-reset"><i class="fa fa-arrow-left"></i>  Back</a> 
    <div class="clearfix"></div>
  </div>
  <div id="contactCitizen"></div>
</div>
<div id="call-portlet">
  <div class="portlet height442 call-portlet">
    <div class="row"> 
      <div class="col-lg-9 col-md-8">
        <div class="p20 pr-0 ">
          <div class="table-responsive scrolldiv">
              ...
              <tbody id="divDetails">
               <!--  <tr>
                  ...
                  <td>
                    Bikash Kumar Panda
                    <div>
                      <small><span class="text-primary">Age: 15 Years, Gender: Male</span></small>
                    </div>
                  </td>
                  <td>9938863395</td>
                  <td>
        ...
        <div class="call-section">
          <div class="call-panel">
            ...
            <div class="text-center">
              <button class="btn btn-danger" type="button" id="btnCallCancel">End Call</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="queue-history d-none">
      <h1>
        <span id="divCallQueue">0</span>
        Callers in Queue
      </h1>
      <div class="coll-cancel text-center">
        <!-- <button class="btn btn-danger" type="button" id="btnCallCancel">End & Cancel Call</button> -->
      </div>
    </div>
    <div class="call-section d-none">
      <!-- <h1 class="text-center" id="callStatus">In Call</h1> -->
      <div class="call-panel">
        <div class="user">
          <img src="<?php echo URL;?>images/user.png">
          <p><?php echo ($_SESSION['adminConsole_FullName'])?$_SESSION['adminConsole_FullName']:'--'; ?></p>
        </div>
        <div class="call-no calling">
          <img src="<?php echo URL;?>images/phone.png">
        </div>
    ...
    <div class="caller-service d-none">
      <div id="serviceDetail">
        ...
        <a id="linkCallHistory" class="btn btn-primary" href="javascript:void(0);">Call History</a>
        <hr>
      </div>
      <h5 class="m-1 text-center">Completed Calls</h5>
      <h2 id="divCallCompleted">0</h2>
    </div>
  </div>
</div>
                        ...
                        <div class="portlet__header2">
                         ...
                         Call Density
                           <div class="indicatorslist">
                              <a title="" href="javascript:void(0)" id="refreshMapIcon" data-toggle="tooltip" data-placement="top" onclick="callRefresh(this); getMapDataDashboard('<?php echo date('Y-m-1');?>','<?php echo date('Y-m-d');?>',1);" data-original-title="Refresh"><i class="icon-reuse"></i></a>
                           </div>
                           <h6>
                        </div>
                        <div class="portlet__body" id="mapContent">
                             <ul class="tab-list">
                            ...
                            <!-- <li ><a class="active" id="outbound"  >Outbound</a></li> -->
                              <!-- <li><a id="inbound"  >Inbound</a></li> -->
                          </ul>
               <div class="form-group row pad-ver">
                            
                              <div class="col-md-9" id="inboundFilter">
                                 <!-- Checkboxes -->
                                <?php if($deptLogin == 0 || $deptLogin== RCMS){ ?>
                                 <div class="checkbox">
                                    <input id="demo-form-checkbox" value="<?php echo(RCMS); ?>" class="magic-checkbox clsInbound" name="mapFilterCheck" type="checkbox">
                                    <label for="demo-form-checkbox">Home</label>
                                 </div>
                               <?php } ?>
                               <?php if($deptLogin == 0 || $deptLogin == PADDY ){ ?>
                                 <div class="checkbox">
                                    <input id="demo-form-checkbox-2" value="<?php echo(PADDY); ?>" class="magic-checkbox clsInbound" name="mapFilterCheck" type="checkbox">
                                    <label for="demo-form-checkbox-2">Health</label>
                                 </div>
                               <?php } ?>
                                 <div class="checkbox" id="checkbox1">
                                    <input id="demo-form-checkbox-3" value="3" class="magic-checkbox clsInbound" name="mapFilterCheck" type="checkbox">
                                    <label for="demo-form-checkbox-3">Call at 14545</label>
                                 </div>
                              </div>

                              <div class="col-md-9" id="outboundFilter">
                                 <!-- Checkboxes -->
                                 <div class="checkbox">
                                    <input id="demo-form-checkbox-4" value="3" class="magic-checkbox clsOutbound" name="mapFilterCheck" type="checkbox">
                                    <label for="demo-form-checkbox-4">By Authority</label>
                                 </div>
                                 <div class="checkbox">
                                    <input id="demo-form-checkbox-5" value="2" class="magic-checkbox clsOutbound" name="mapFilterCheck" type="checkbox">
                                    <label for="demo-form-checkbox-5">By Contact Center</label>
                                 </div>
                                 <!-- <div class="checkbox" id="checkbox1" style="display: none;">
                                    <input id="demo-form-checkbox-3" value="3" class="magic-checkbox1" name="mapFilterCheck1" type="checkbox">
                                    <label for="demo-form-checkbox-3">Call at 14545</label>
                                 </div> -->
                              </div>
                           </div>
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-20">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-17">
...
<path  class="st0 dist-block"  id="17" name="kandhamal"  style="cursor:pointer;"  d="M287.9,298v-2.8l0.8-0.9h0.8l0.8-0.8v-0.8l0.8-0.9h-1.6l-0.9-0.9l-0.9,0.1l-1.6-1.6v-1.6l-1.7-0.2v-0.9
  ...
  l4.9,0.1l0.9,0.8l1.6,0.9h0.8"/></a>
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-23">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-4">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-6">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-14">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-10">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-8">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-13">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-18">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-15">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-31">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-5">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-30">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-3">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-16">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-7">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-26">
<path class="st0 dist-block"  id="26" name="nuapada" style="cursor:pointer;"  d="M102.3,166h-0.9v-0.8l-0.8,0.8l0.2,3.4l-0.9,0.9l0.9,1.7l0.3,13.8l1.7,1.7l0.1,6.7l-0.8,0.9l0.1,2.3h-0.8
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-24">
<path class="st0 dist-block"  id="24" name="nabarangpur" style="cursor:pointer;"  d="M63.2,244.4h-2.3v0.8H60l-1.6,1.6l-0.8,1.7l-0.9,0.8h-1.4l-0.9,0.9l0.1,2.5l-0.8,0.8v1.6L53,256v1.6l-0.8,0.9
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-21">
<path class="st0 dist-block"  id="21" name="koraput" style="cursor:pointer;"  d="M89.7,342.5l-0.7-0.1l0.7,0.9l0.1,2.5l-0.7,0.8l2.4,5.1l0.2,7.4h0.9l0.1,3.2H91l-1.5-0.8h-1.7l-1.6,0.8
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-22">
<path class="st0 dist-block"  id="22" name="malkangiri" style="cursor:pointer;"  d="M59.1,397.5l0.8,0.9l0.1,0.8h-1.6l-0.8,0.9h-0.8l-0.9,0.6h-0.8v1.8l-0.8,0.8v0.9h-0.7l-3.3,3.3l-1.6,0.8
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-28">
...
<g>
  <g>
    <g>
      <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-29">
...
<a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-9">
<path class="st0 dist-block"  id="9" name="debgarh"  style="cursor:pointer;"  d="M321.6,84.4l-0.8,0.4l0.9,1.3l-0.5,1l-1.3,0.3c0,0-1,0.6-1.1,0.6s-4.7,0-4.7,0l-1.7,1.7l-4.1,0.1l-0.8-0.9v0.8
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-12">
<path  class="st0 dist-block"  id="12" name="ganjam"  style="cursor:pointer;"  d="M288.5,298l1.1,0.1v0.8v0.8h0.8h1.6l0.8-0.8v-0.8l1.6-1.6v-0.8l0.9-0.9v0.8l0.9,0.8l0.1,0.8h5.8v0.8h0.8
 ...
 <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-11"> 
<path class="st0 dist-block"  id="11" name="gajapati"  style="cursor:pointer;"  d="M244.3,356.4l2.1-2.4h0.9v-0.8l0.8-0.7l1.6-0.9v0.8l0.9,0.8h0.9v0.8l0.8,0.9l0.8,0.1l1.7,1.6l2.5-0.1l-0.1-2.6
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-27">
<path class="st0 dist-block"  id="27" name="Puri"  style="cursor:pointer;"  d="M468,245.2l-1.3,0.1l-1.7,0.9l-0.8-0.9h-4l-0.9-0.8h-0.9l-0.6,0.8h-0.9v0.9l-1.6-0.2l-0.1-2.5l-1.6,0.1
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-25">
<path class="st0 dist-block"  id="25" name="nayagarh"  style="cursor:pointer;"  d="M333.2,204.2l-0.7,0.9v0.8l-0.8,1.6l-0.8,2.5l-0.8,0.8l0.9,0.9l0.9,1.6H331l-0.8,0.8h-0.9l-0.8,0.9h-4.1
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-19">
<path class="st1 dist-block block-kj"  id="19" name="keonJhar"  style="cursor:pointer;"  d="M491.7,133.9V133h0.7l0.8-0.7h-0.9v-0.8l-0.7-0.9v-0.8h-0.9l-0.1-2.4v-0.9l0.9-0.8l-0.1-0.8l-0.9-1v-0.7h-0.8
  ...
  <a href="<?php echo URL;?>viewMapDetails/" target="_blank" id ="link-2">
<path class="st2 dist-block"  id="2" name="anugul"  style="cursor:pointer;"  d="M398.1,139.1l-1.6-1.7l-0.1-1.6l0,0v-0.1l-1.5-1.3l0,0l-0.3-0.3c0,0-0.5-0.3-1.1-0.7l0,0l0,0
  ...
  l1.5-1.6l4.2-0.2l0.8-0.9c0,0,0.8-1.6,0.8-2.6s0.8-2.3,0.8-2.3v-2.3l2.6-1.4h3.5l1.1-0.6l6.6,0.1L398.1,139.1z"/>
                                ...
                                <table class="table">
                                  <tr><!-- <th id="srvcTransactionH">Citizen Contact</th> --><th id="callsDetail" >Calls Made</th><!-- <th id="feedbackH" >Feedbacks</th> --></tr>
                                  <tr><!-- <td id="srvcTransaction" >0</td> --><td id="total-cal-blue">0</td><!-- <td id="feedback">0</td> --></tr>
                                </table>
                            </div>
   <div class="text-center">
          ...
          <div class="row">
            ...
            <div class="col-sm-4">
              <div class="input-group ">
                  ...
                  <input type="text"   name="vchFromDate" id="vchFromDate" class="form-control "   autocomplete="off" readonly="readonly" value="<?php echo $firstWeekfilter;?>" >
                  <span class="input-group-addon"><i class="icon-calendar1"></i></span>
              </div>
            </div>
            ...
            <div class="col-sm-2">
                <button class="btn btn-primary btn-sm" type="button" onclick="getDataInboundOutbound()"><i class="icon-filter1"></i></button>
                 ...
                 <!-- <div class="indicatorslist"> -->
                  <a title="" href="javascript:void(0)" class="btn btn-info btn-sm" id="refreshIcon" data-toggle="tooltip" data-placement="top" onclick="getDataInboundOutbound();" data-original-title="Refresh"><i class="icon-reuse"></i></a>
                           <!-- </div> -->
            </div>
          </div>
...
From Contact Center
 ...
 <?php 
        ...
        $citizenHomeContact['callType']          = 2;
        #$queryString['outBoundId']          = $outBoundId;
        #$queryString['deptSel1']          = $deptSel1;
        #$queryString['fromDate']          = $fromDate;
        #$queryString['toDate']          = $toDate;
        $citizenHomeContact['deptLogin']          = $deptLogin;
        $citizenHomeContact['deptFilter']  = RCMS;

        // $queryString['newdeptSel1']          = $newdeptSel1;
        $citizenHomeContactQuery   = $obj->encrypt(json_encode($citizenHomeContact));

        $citizenHealthContact['callType']          = 2;
        $citizenHealthContact['deptLogin']          = $deptLogin;
        $citizenHealthContact['deptFilter']  = PADDY;
        $citizenHealthContactQuery   = $obj->encrypt(json_encode($citizenHealthContact));

        $citizenContact['callType']          = 2;
        $citizenContact['deptFilter']= '14545';
        $citizenContact['deptSel1']= 2;
        // $citizenContact['deptCC']= '14545';
        $citizenContactQuery   = $obj->encrypt(json_encode($citizenContact));

        $directCall['callType']          = 1;
        $directCall['deptFilter']= '';
        $directCall['outBoundId']= '3';
        ...
        $ccOutCall['callType']          = 1;
        $ccOutCall['deptFilter']= '';
        $ccOutCall['outBoundId']= '2';
        $ccOutCall['intCallThrough']= '1';
        $ccOutCallQuery   = $obj->encrypt(json_encode($ccOutCall));

        $todayAuthCall['callType']          = 1;
        $todayAuthCall['deptFilter']= '';
        $todayAuthCall['outBoundId']= '3';
        $todayAuthCall['intCallThrough']= '0';
        $todayAuthCall['fromDate']= date('Y-m-d');
        $todayAuthCall['toDate']= date('Y-m-d');
        // $citizenContact['deptSel1']= 2;
        // $citizenContact['deptCC']= '14545';
        $todayAuthCallQuery   = $obj->encrypt(json_encode($todayAuthCall));

        $weekAuthCall['callType']          = 1;
        $weekAuthCall['deptFilter']= '';
        $weekAuthCall['outBoundId']= '3';
        $weekAuthCall['intCallThrough']= '0';
        $day = date('w');
        $weekAuthCall['fromDate']= date('Y-m-d', strtotime('-'.$day.' days'));
        $weekAuthCall['toDate']= date('Y-m-d', strtotime('+'.(6-$day).' days'));
        $weekAuthCallQuery   = $obj->encrypt(json_encode($weekAuthCall));

        $monthAuthCall['callType']          = 1;
        $monthAuthCall['deptFilter']= '';
        $monthAuthCall['outBoundId']= '3';
        $monthAuthCall['intCallThrough']= '0';
        $day = date('w');
        $monthAuthCall['fromDate']= date('Y-m-01');
        $monthAuthCall['toDate']= date('Y-m-d');
        $monthAuthCallQuery   = $obj->encrypt(json_encode($monthAuthCall));




        $todayContact['callType']          = 2;
        $todayContact['deptFilter']= '14545';
        $todayContact['deptSel1']= 2;
        ...
        $weekContact['deptFilter']= '14545';
        $weekContact['deptSel1']= 2;
        $weekContact['fromDate']= date('Y-m-d', strtotime('-'.$day.' days'));
...
<div class="left-details" id="depart-home-health-cc-content">
            <h4><i class="icon-users"></i> Citizen Contact  <span><a href="javascript:void(0);" id="depart-home-health-cc">0</a></span></h4>
          <ul>
            
            <li>RCMS <span><a href="" id="depart-Rcm">0</a></span></li>

            <li>Paddy Procurement <span><a href="" id="depart-Peddy">0</a></span></li>

          </ul>


          <input type="hidden" id="outBoundId" name="outBoundId" value="2,3">
          <input type="hidden" id="deptSel" name="deptSel" value="0">
          <input type="hidden" id="deptSel1" name="deptSel1" value="2">
          <input type="hidden" id="callType" name="callType" value="0">

 
</div>

<div class="left-details" id="auth-direct-cc-content">
            <h4><i class="icon-phone-outgoing1"></i>Calls Made (Authority)<span><a href="javascript:void(0);" id="auth-direct-cc">0</a></span></h4>
            <ul class="twoli">            
              <li>Direct Calls <span><a href=""  id="auth-direct" >0</a></span></li>
              <li>Through CC <span><a href="" id="auth-cc">0</a></span></li>
            </ul>
</div>
<div class="left-details" id="cc-inbound-outbound-content" >
    <h4><i class="icon-phone-outgoing1"></i>Contact Center<span><a href="" id="cc-inbound-outbound" >0</a></span></h4>
    <ul class="twoli">
      <li>Outbound Calls <span><a href=""  id="cc-outbound" >0</a></span></li>
    </ul>
</div>
<div class="left-details" id="auth-outbound-avg-content" >
      <h4><i class="icon-phone1"></i>Authority Daily Average Call (outbound) <span> <a href="javascript:void(0);" id="auth-outbound-avg" >0</a></span></h4>
      <ul> 
        <li>Today <span><a id="auth-outbound-today" >0</a></span></li>
        <li>Week <span><a id="auth-outbound-week">0</a></span></li>
        <li>Month <span><a id="auth-outbound-month">0</a></span></li>
      </ul>
</div>
<div class="left-details" id="auth-outbound-cc-avg-content">
      <h4><i class="icon-phone1"></i>CC Daily Average Call (outbound) <span> <a href="javascript:void(0);">0</a></span></h4>
      <ul> 
        <li>Today <span><a href="" id="auth-outbound-cc-today">0</a></span></li>
        <li>Week <span><a href="" id="auth-outbound-cc-week">0</a></span></li>
        <li>Month <span><a href="" id="auth-outbound-cc-month">0</a></span></li>
      </ul>
</div>
...
<script>
  ...
  function showFeedbackQuestionAnswer(intDepartmentId, intServiceId, intFeedbackRecId, cardNo = 0){
    ...
    $.ajax({
      ...
      url: URL + '/proxy',
      dataType: "json",
      data: {method: 'showFeedbackQuestionAnswer',intDepartmentId:intDepartmentId,intServiceId:intServiceId,cardNo:cardNo,csrf_token_id:csrf_token_val},
...
function AuthCallCancel(vchOutBoundId, OutBoundType){
  $.ajax({
    type: "POST",
    url: URL + '/proxy',
    dataType: "json",
    data: {method: 'AuthCallCancel',vchOutBoundId:vchOutBoundId, OutBoundType:OutBoundType,csrf_token_id:csrf_token_val},
    success: function (data) {
      // console.log(data);
      location.reload();
    }
...
function getCallBackRequestSession(vchOutBoundId, OutBoundType){
    $.ajax({
      type: "POST",
      url: URL + '/proxy',
      dataType: "json",
      data: {method: 'getCallBackRequestSession',vchOutBoundId:vchOutBoundId, OutBoundType:OutBoundType,csrf_token_id:csrf_token_val},
      success: function (data) {
        console.log(data);
      }
    })
...
function getCallBackRequest(vchOutBoundId, OutBoundType){
    $.ajax({
      type: "POST",
      url: URL + '/proxy',
      dataType: "json",
      data: {method: 'getCallBackRequest',vchOutBoundId:vchOutBoundId, OutBoundType:OutBoundType,csrf_token_id:csrf_token_val},
      success: function (data) {
        // console.log(data.result);
        // $('#contactCitizen').hide();
        // $('#viewDetails').show();
        ...
        $('#btnCallCancel').attr('onclick', "AuthCallCancel('"+ vchOutBoundId +"','"+OutBoundType+"')");

        var res = data.result;
        // $('#divCallQueue').html(res.callInQueue);
        $('#divDetails').html(data.vchTableData);
        ...
        if(res.callInConnecting > 0){
            ...
            $('#callName').html(res.vchConnectingName);
            $('#callMobile').html(res.vchConnectingMobile);
            $('#btnCallCancel').html('End Call');
            $('#btnCallCancel').attr('class','btn btn-danger');
            ...
            // $('#linkCallHistory').attr('onclick','DisplayCallLog('+ res.vchMobile +')');
          // $('#divServiceDetails').html(data.result);    
            ...
            $('#callName').html('');
            $('#callMobile').html('');
            $('#btnCallCancel').html('Back to Home');
            $('#btnCallCancel').attr('class','btn btn-warning');
            $('#btnCallCancel').attr('onclick','location.reload()');
            // $('#callDepartment').html();
            // $('#callService').html();
            // $('#callDistrict').html();
            /*setTimeout(function () {
            ...
            $('#callStatus').html('data.vchStatus');
            $('#btnCallCancel').html('End Call');
            $('#btnCallCancel').attr('class','btn btn-danger');
            if(res.vchCurrentStatus != 0){
              if(res.intStatus == 1)
                $('.call-no').addClass('calling');
              else
                $('.call-no').removeClass('calling');

              $('#callStatus').html(res.vchCurrentStatus);
            }else{
...
function assignCallBackRequest(vchOutBoundId, OutBoundType){
    $.ajax({
      type: "POST",
      url: URL + '/proxy',
      dataType: "json",
      data: {method: 'assignCallBackRequest',vchOutBoundId:vchOutBoundId, OutBoundType:OutBoundType,csrf_token_id:csrf_token_val},
      success: function (data) {
        console.log()
      }
    });
...
function directCallCM(vchOutBoundId, OutBoundType){
    $.ajax({
      type: "POST",
      url: URL + '/proxy',
      dataType: "json",
      data: {method: 'directCallCM',vchOutBoundId:vchOutBoundId, OutBoundType:OutBoundType,csrf_token_id:csrf_token_val},
      success: function (data) {
        window.location.href = URL+'/outboundDataMis';
      }
    });
}
function resetCallBackRequest(vchOutBoundId, OutInBoundType){
  $.ajax({
    type: "POST",
    url: URL + '/proxy',
    dataType: "json",
    data: {method: 'resetCallBackRequest',vchOutBoundId:vchOutBoundId, OutInBoundType:OutInBoundType,csrf_token_id:csrf_token_val},
    success: function (data) {
      $('#call-portlet').hide();
      $('.caller-list').hide();
      $('.citizen-connect-section').show();
...
function unAssignCallBackRequest(vchOutBoundId, OutInBoundNotType){
  $.ajax({
    type: "POST",
    url: URL + '/proxy',
    dataType: "json",
    data: {method: 'unAssignCallBackRequest',vchOutBoundId:vchOutBoundId, OutInBoundNotType:OutInBoundNotType,csrf_token_id:csrf_token_val},
    success: function (data) {
...
function pullData(intDepartmentId, intServicesId, intDistrictId, intGender, vchAgeGroup, intCallerId, vchCitizenFromDate, vchCitizenToDate, intHospitalPolice){
        ...
        loadOutBoundDataCm('pull','contactCitizen',2,'',numberofrec,vchDepartmentId,intServiceId,district,gender,agegroup, vchCitizenFromDate, vchCitizenToDate, intHospitalPolice);
...
$('body').on('click', '#make-call', function(){
    ...
    var OutBoundType      = '';
    var OutBoundNoteId    = '';
    var OutInBoundNotType = '';
    $('.chkCall').each(function(){
        if($(this).is(':checked')){
            OutBoundId        += $(this).val()+',';
            OutBoundType      += $(this).parent().find('.intType').val()+',';
          }
    });
    $('.chkCall').each(function(){
        if($(this).is(':not(:checked)')){
            OutBoundNoteId    += $(this).val()+',';
            OutInBoundNotType += $(this).parent().find('.intType').val()+',';
          }
    });
    // console.log(OutBoundNoteId);return false;
    ...
    // getCallBackRequestSession(OutBoundId, OutBoundType);
    assignCallBackRequest(OutBoundId, OutBoundType);
    getCallBackRequest(OutBoundId, OutBoundType);

    if($('.chkCall:not(:checked)').length > 0){
      unAssignCallBackRequest(OutBoundNoteId, OutInBoundNotType);
    }
    setInterval(function(){ 
        getCallBackRequest(OutBoundId, OutBoundType);
    }, 5000)
})
$('body').on('click', '#make-call-direct', function(){
    var OutBoundId        ='';
    var OutBoundType      = '';
    var OutBoundNoteId    = '';
    var OutInBoundNotType = '';
    ...
    $('#make-call-direct').attr('disabled','disabled');
    directCallCM(OutBoundId, OutBoundType);
...
$('body').on('click', '#generate-action-shirt', function(){
    var OutBoundId        ='';
    var OutBoundType      = '';
    var OutBoundNoteId    = '';
    var OutInBoundNotType = '';
    ...
    generateActionSheet(OutBoundId);
    // directCallCM(OutBoundId, OutBoundType);
})
...
$('body').on('click', '#excelExport', function(){
   ...
   // alert(intDepartmentId);
    $('.chkCall').each(function(){
        if($(this).is(':checked')){
            tableRow            += '<tr>';
            OutBoundId          = $(this).val();
            OutBoundTypeId      = $(this).parent().find('.intType').val();
            fetchedRow          = $('#bookMarked'+OutBoundId+'_'+OutBoundTypeId).html();

            fetchedRow          = fetchedRow.replace(/<A[^>]*>|<\/A>/g, "");//remove if u want links in your table
            fetchedRow          = fetchedRow.replace(/<img[^>]*>/gi,""); // remove if u want images in your table
            fetchedRow          = fetchedRow.replace(/<input[^>]*>|<\/input>/gi, ""); // reomves input params
            fetchedRow          = fetchedRow.replace(/<label[^>]*>|<\/label>/gi, ""); // reomves label params
            fetchedRow          = fetchedRow.replace(/--/gi,"");  // reomves label params
            fetchedRow          = fetchedRow.replace(/<button[^>]*>|<\/button>/gi, ""); // reomves Button params
            fetchedRow          = fetchedRow.replace(/<a[^>]*>|<\/a>/gi, ""); // reomves Button params
            fetchedRow          = fetchedRow.replace(/view/gi, ""); // reomves Button params
            tableRow            += fetchedRow;
            if(intDepartmentId==0){
              tableRow            += ' <td></td> <td></td><td></td>';
              tableRow            += ' <td></td><td></td>';
            }else if(intDepartmentId==12){//health
              tableRow            += ' <td></td> <td></td><td></td>';
            }
            else if(intDepartmentId==14){//home
              tableRow            += ' <td></td><td></td>';
            }            
    ...
    $('.addTr:last').closest('tr').after(tableRow);
    //generateActionSheet(OutBoundId);
    // directCallCM(OutBoundId, OutBoundType);
    setTimeout(function(){
            exportToExcel()
...
$('body').on('click', '#chkAll', function(){
     // $(document).find('.chkAll').click(function(){ alert();
				if($(this).is(':checked'))
					$('.chkCall').attr('checked',true);
				else
					$('.chkCall').attr('checked',false);
			});
...
$('body').on('click', '#make-reset', function(){
    ...
    $('.chkCall').each(function(){
        OutBoundId        += $(this).val()+',';
        OutInBoundType    += $(this).parent().find('.intType').val()+',';
    });
    // console.log(OutBoundId);
    // console.log(OutInBoundType);
    resetCallBackRequest(OutBoundId, OutInBoundType);

})
$(document).ready(function(){
    <?php 
    if(isset($_SESSION['AuthCallStart']) && isset($_SESSION['AuthCallStartType'])){
    ?>
    getCallBackRequest('<?php echo $_SESSION['AuthCallStart'] ?>', '<?php echo $_SESSION['AuthCallStartType'] ?>');
    ...
    ?>
    //getDepartmentList('intDepartmentId', '<?php echo ($_SESSION["adminConsole_DeptId"])?$_SESSION["adminConsole_DeptId"]:DEPART_FSCW ?>');
    getDeptWiseService('intServicesId', '<?php echo ($_SESSION["adminConsole_DeptId"])?$_SESSION["adminConsole_DeptId"]:DEPART_FSCW ?>',0)
    <?php
    if(isset($_SESSION["adminConsole_DeptId"]) && $_SESSION["adminConsole_DeptId"] != 0){
     ?>
      getDeptWiseService('intServicesId','<?php echo $_SESSION["adminConsole_DeptId"] ?>',0);
    <?php } ?>
    <?php if($_SESSION['adminConsole_agency']==0){ ?>
...
$('#searchbtn').on("click",function(){
    ...
    var vchCitizenFromDate  = ($('#vchCitizenFromDate').val())?$('#vchCitizenFromDate').val():'';
    var vchCitizenToDate    = ($('#vchCitizenToDate').val())?$('#vchCitizenToDate').val():'';
    var intHospitalPolice   = ($('#intHospitalPolice').val())?$('#intHospitalPolice').val():'';
    ...
    if(intAgeGroup == 1){
        ...
        vchAgeGroup     = '18,60';
    }else if(intAgeGroup == 4){
        vchAgeGroup     = '60,100';
    }else{
        vchAgeGroup     = '';
    }
    pullData(intDepartmentId, intServicesId, intDistrictId, intGender, vchAgeGroup, intCallerId, vchCitizenFromDate, vchCitizenToDate, intHospitalPolice);
...
function getDataInboundOutbound(){
  ...
  function callRefresh(obj){
      ...
      $('.clsInbound').prop('checked',true);
      $('.clsOutbound').prop('checked',true);
  } 
...
<script language="javascript">
  ...
  function getDailyAvgCCCall(){
    $("#auth-outbound-cc-avg-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getDailyAvgCCCall',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#auth-outbound-cc-avg").html(data.AVGCNT);
          if(parseInt(data.TODAYCNT)>0){
            $("#auth-outbound-cc-today").html(data.TODAYCNT).attr("href", data.todayContactQuery).attr('target','_blank');
          }
          if(parseInt(data.WEEKCNT)>0){
            $("#auth-outbound-cc-week").html(data.WEEKCNT).attr("href", data.weekContactQuery).attr('target','_blank');
          }
          if(parseInt(data.MONTHCNT)>0){
            $("#auth-outbound-cc-month").html(data.MONTHCNT).attr("href", data.monthContactQuery).attr('target','_blank');
          }
          $("#auth-outbound-cc-avg-content").unblock();
        }
    });
}
  function getDailyAvgCallInbound(){
    $("#cc-inbound-avg-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getDailyAvgCallInbound',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#cc-inbound-avg").html(data.AVGCNT);
          if(parseInt(data.TODAYCNT)>0){
            $("#cc-inbound-today").html(data.TODAYCNT).attr("href", data.todayContactQuery).attr('target','_blank');
          }
          if(parseInt(data.WEEKCNT)>0){
            $("#cc-inbound-week").html(data.WEEKCNT).attr("href", data.weekContactQuery).attr('target','_blank');
          }
          if(parseInt(data.MONTHCNT)>0){
            $("#cc-inbound-month").html(data.MONTHCNT).attr("href", data.monthContactQuery).attr('target','_blank');
          }
          $("#cc-inbound-avg-content").unblock();
        }
    }); 
  }
  function getDailyAvgCall(){
    $("#auth-outbound-avg-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getDailyAvgCall',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#auth-outbound-avg").html(data.AVGCNT);
          if(parseInt(data.TODAYCNT)>0){
            $("#auth-outbound-today").html(data.TODAYCNT).attr("href", data.todayContactQuery).attr('target','_blank');
          }
          if(parseInt(data.WEEKCNT)>0){
            $("#auth-outbound-week").html(data.WEEKCNT).attr("href", data.weekContactQuery).attr('target','_blank');
          }
          if(parseInt(data.MONTHCNT)>0){
            $("#auth-outbound-month").html(data.MONTHCNT).attr("href", data.monthContactQuery).attr('target','_blank');
          }
          $("#auth-outbound-avg-content").unblock();
        }
    });
}
function getCC(){
   $("#cc-inbound-outbound-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getCC',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#cc-inbound-outbound").html(data.totalCC);
          // if(parseInt(data.inbound)>0){
          //   $("#cc-inbound").html(data.inbound).attr("href", data.citizenContactQuery).attr('target','_blank');
          // }
          if(parseInt(data.outbound)>0){
            $("#cc-outbound").html(data.outbound).attr("href", data.ccCallQuery).attr('target','_blank');
          }
          $("#cc-inbound-outbound-content").unblock();
        }
    });
}
function getCallSent(){
  $("#auth-direct-cc-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getCallSent',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#auth-direct-cc").html(data.totalCallSent);
          if(parseInt(data.auth)>0){
            $("#auth-direct").html(data.auth).attr("href", data.directCallQuery).attr('target','_blank');
          }else{
            $("#auth-cc").html(data.auth).attr("href", 'javascript:void(0);').attr('target','_blank');
          }
          if(parseInt(data.callCenter)>0){
            $("#auth-cc").html(data.callCenter).attr("href", data.ccCallQuery).attr('target','_blank');
          }else{
            $("#auth-cc").html(data.callCenter).attr("href", 'javascript:void(0);').attr('target','_blank');
          }
           $("#auth-direct-cc-content").unblock();
        }
    });
}
function getPeopleDepartment(){ 
    $("#depart-home-health-cc-content").block({ message: '<img style="display: block;margin: 0 auto;" src="'+URL+'/img/loader-white1.gif" />' });
    $.ajax({
        type: "POST",
        url: URL + '/proxy',
        dataType: "json",
        data: {method: 'getPeopleDepartment',csrf_token_id:csrf_token_val },
        success: function (data) {
          $("#depart-home-health-cc").html(data.totalPeople);
          if(parseInt(data.totalHome)>0){
            $("#depart-Rcm").html(data.totalHome).attr("href", data.citizenHealthContactQuery).attr('target','_blank');
          }
          if(parseInt(data.totalHealth)>0){
            $("#depart-Peddy").html(data.totalHealth).attr("href", data.citizenHomeContactQuery).attr('target','_blank');
          }
          // if(parseInt(data.totalCalls)>0){
          //   $("#depart-cc").html(data.totalCalls).attr("href", data.citizenHealthContactQuery).attr('target','_blank');
          // }
           $("#depart-home-health-cc-content").unblock();
        }
    });
}
</script>
