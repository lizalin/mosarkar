<?php 
    ...
    $intDirectFeedbackStatus   = $arrDecrypt['intDirectFeedbackStatus'];
...
//print_r($deletePriv);exit;
	...
	if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
	{
		...
		unset($_SESSION['paging']);
	     $vchDepartmentId = ($_POST['intDepartmentId']>0 && $_POST['search']=='search')?$_POST['intDepartmentId']:0;
	     ...
	     $Directstatus          = ($_POST['intCallStatus']!='')?$_POST['intCallStatus']:$intDirectFeedbackStatus; 
    ...
    if($isPaging==0){
        //if($type==1){ $action='PG'; } elseif ($type==2) { $action='PGI'; } else { $action='PGF'; }
            $action='VAFP';
        $dataArr['intDirectFeedbackStatus']        = $Directstatus;
