...
//print_r($deletePriv);exit;
    ...
    if($isPaging==0){
        $result        = $obj->viewOutboundMis('VOMR',$dataArr);
    }     
    else
    {
        $intPgno    = 1;
    ...
    /*For Total Recodes*/
    //$TotalCountFeedback         = $obj->viewOutboundMis('VOT',$dataArr);
    //$totalResultRow             = $TotalCountFeedback->fetch_array();


    $totalResult                = $obj->viewOutboundMis('VOMR',$dataArr);
    $intTotalRec                = $totalResult->num_rows;      

    // print_r($totalResultRow);exit;
