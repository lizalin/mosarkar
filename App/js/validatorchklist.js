...
function maxLength(controlId,ctrlLen,fieldName)
{
	...
	{
		viewAlert(fieldName+' can not more than '+ctrlLen+' charater !!!');
		$('#'+controlId).focus();
		return false;
	}
	return true;
...
if(txtVal!='')
{
	...
	{
		viewAlert("Enter Only Numeric values");
		$('#'+controlId).focus();
		return false;
	}
}
...
function validateCharNumber(controlId, fieldName)
{
	...
	if(txtVal!='')
	{
		...
		{
			viewAlert(fieldName+' accept only Number and Character');
			$('#'+controlId).focus();
			return false;
		}
	}
...
function whiteSpaceLast(controlId)	
{
	...
	if(lastChar==' ')
	{
		viewAlert('Please remove space from last');
		$('#'+controlId).focus();
		return false;
	}
	return true;
...
function checkSpecialChar(controlId)
{
	...
	for(var i=0; i<splArr.length; i++)
	{
		if (str.indexOf(splArr[i]) > 0) {
			viewAlert("Special character "+splArr[i]+" is not allowed !!!");
			$('#'+controlId).focus();
			return false; 
		}			
	}
...
function validEmail(controlId) 
{
	...
	if(email!='')
	{
		...
		{
			viewAlert("Please enter a valid Email ID");
			$('#'+controlId).focus();
			return false;
		}
	}
...
function compareDate(firstControl, secondControl, field1Name, field2Name,msg)
{
	...
	if(firstDate>secondDate)
	{
		...
		viewAlert(field1Name+" can not be greater than "+field2Name);
		else
			viewAlert(msg);
		$('#'+secondControl).focus();
		return false;
...
function compareCurDate(controlId, fieldName, flag)
{
	...
	if(flag.toLowerCase()=='g')
	{
		...
		{
			viewAlert(fieldName+" can not be greater than current date");
			$('#'+controlId).focus();
			return false;
		}
		else
	...
	else
	{
		...
		{
			viewAlert(fieldName+" can not be less than current date");
			$('#'+controlId).focus();
			return false;
		}
		else
...
//=============== Function to compare date with current date ==============
function compareCurDate2(controlId, fieldName, flag)
{
	//================ set flag as 'g' for not greater than current date and 'l' for not less than current date ========
	var fieldDate	= $('#'+controlId).val();
	var splFieldDt	= fieldDate.split("-");
